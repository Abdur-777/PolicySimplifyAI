# app.py — PolicySimplify AI (Day 6)
...
if PASSCODE:
    ...
    if not st.session_state["auth_ok"]:
        ...
        if st.button("Enter"):
            if code == PASSCODE:
                st.session_state["auth_ok"] = True; st.experimental_rerun()
            else:
...
try:
    ...
    if removed: log.info("Purged old rows: %s", removed)
except Exception as e:
    removed = 0; log.warning("Purge failed: %s", e)

# Session state
if "store" not in st.session_state: st.session_state["store"] = SimpleFAISS.load(VECTOR_DB_NAME)
if "policy_cards" not in st.session_state: st.session_state["policy_cards"] = []
if "loaded_persisted" not in st.session_state:
    persisted = load_cards(limit=500)
    if persisted: st.session_state["policy_cards"].extend(persisted); log.info("Loaded %s cards", len(persisted))
...
with st.sidebar:
    ...
    if uploaded is not None and st.button("Process uploaded PDF"):
        try:
            content = uploaded.read(); source_name = uploaded.name
            st.session_state["_ingest_uploaded"] = (source_name, content)
            log_event("upload", source_name); log.info("Queued upload: %s", source_name)
        except Exception as e:
            st.error(f"Could not read uploaded file: {e}"); log.exception("Upload read failed")

    url = st.text_input("Or fetch from URL (PDF)", placeholder="https://example.gov.au/policy.pdf")
    if st.button("Fetch & process URL"):
        if not url.strip(): st.warning("Please paste a valid URL to a PDF.")
        else:
            try:
                with st.spinner("Downloading PDF..."):
                    r = requests.get(url, timeout=20); r.raise_for_status()
                st.session_state["_ingest_url"] = (os.path.basename(url) or "Policy_From_URL.pdf", r.content)
                st.success("Downloaded. Processing…"); log_event("url", url); log.info("Downloaded URL: %s", url)
            except Exception as e:
                st.error(f"Failed to fetch PDF: {e}"); log.exception("URL fetch failed")

    st.markdown("---")
    if st.button("Use example policy"):
        st.session_state["_ingest_example"] = True
        log_event("example", "Example_Waste_Services_Policy.txt"); log.info("Example queued")

    st.markdown("---")
    if st.button("🗑️ Clear session & database"):
        st.session_state["policy_cards"] = []; st.session_state["store"] = SimpleFAISS.load(VECTOR_DB_NAME)
        try: clear_all()
        except Exception as e: st.warning(f"DB clear error: {e}"); log.exception("DB clear failed")
        st.success("Cleared."); log_event("admin", "clear_all"); log.info("Cleared session+DB")
    ...
    if ALLOW_DB_EXPORT:
        try:
            with open(os.getenv("DB_PATH", "./policy.db"), "rb") as f:
                st.download_button("Download SQLite (policy.db)", data=f.read(), file_name="policy.db",
...
def process_policy(source_name: str, *, file_bytes: bytes | None = None, raw_text: str | None = None):
    ...
    with st.spinner("Reading policy..."):
        ...
        text = (text or "").strip()
        if not text:
            st.warning("No text found (maybe a scanned PDF without OCR)."); log.warning("Empty text after extraction")
            return
        if len(text) > TEXT_CAP:
            log.info("Truncate text from %s to %s chars", len(text), TEXT_CAP)
            text = text[:TEXT_CAP]
        text = scrub(text)  # PII redaction before embedding
    ...
    with st.spinner("Generating summary, checklist & risk..."):
        ...
        try:
            ...
            st.error(f"OpenAI/Azure error: {e}"); log.exception("LLM call failed"); return
        ...
        try: save_card(card)
        except Exception as e: st.warning(f"DB persist error: {e}")
...
def _count_bullets(text: str) -> int:
    ...
    for ln in (text or "").splitlines():
        s = ln.strip()
        if not s: continue
        if s.startswith(("-", "*", "•")): cnt += 1
        elif (len(s) > 1 and s[0].isdigit() and s[1] == "."): cnt += 1
    return cnt
...
if not st.session_state["policy_cards"]:
    ...
    with colA: risk_filter = st.multiselect("Filter by risk", ["High","Medium","Low"], default=["High","Medium","Low"])
    with colB: high_only = st.checkbox("Show only High risk", value=False)
    ...
    if len(view_df) > 0:
        ...
        st.markdown("**Risk explainer:**"); st.write(row["Risk explainer"])
    ...
    if st.toggle("Show structured tasks", value=False):
        ...
        else: st.info("No structured tasks yet for current selection.")
    ...
    with st.expander("Email this Audit Pack"):
        ...
        if st.button("Send Audit Pack", key="send_email_btn"):
            try:
                ...
                send_email_with_pdf(to_addr=to, subject=f"[{COUNCIL_NAME}] PolicySimplify Audit Pack",
                                    body="Attached: Audit Pack generated by PolicySimplify AI.",
                                    pdf_bytes=pdf_bytes, filename="PolicySimplify_Audit_Pack.pdf")
                st.success("Sent!"); log_event("email", f"to={to}"); log.info("Audit Pack emailed to %s", to)
            except Exception as e:
                st.error(f"Email failed: {e}"); log.exception("Email send failed")
...
if st.button("Get answer") and q.strip():
    ...
    if not snippets: st.info("No context found yet. Upload a policy first.")
    else:
        ...
        with st.expander("Sources"):
            for i, (score, doc) in enumerate(hits, 1):
                ...
                st.markdown(f"**{i}. {src}** (score {score:.2f})"); st.code(doc["text"][:700])
        try: log_event("qa", q); log.info("Q&A answered: %s", q)

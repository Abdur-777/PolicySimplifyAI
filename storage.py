...
def _conn():
    ...
    con.execute(SCHEMA); con.execute(EVENTS_SCHEMA)
    return con
    ...
    with con:
        con.execute(
            ...
            (card.get("policy") or card.get("source_name") or "Unknown",
             card.get("summary",""), card.get("checklist",""),
             card.get("risk","Medium"), card.get("risk_explainer",""),
...
def load_cards(limit: int = 500) -> List[Dict[str, Any]]:
    con = _conn()
    rows = con.execute("SELECT source_name, summary, checklist, risk, risk_explainer, created_at "
                       "FROM policy_cards ORDER BY created_at DESC LIMIT ?", (limit,)).fetchall()
    con.close()
    return [{"policy": r[0], "summary": r[1], "checklist": r[2], "risk": r[3], "risk_explainer": r[4], "created_at": r[5]} for r in rows]

def clear_all() -> None:
    con = _conn()
    with con:
        con.execute("DELETE FROM policy_cards"); con.execute("DELETE FROM events")
    con.close()
...
def recent_events(limit: int = 5):
    con = _conn()
    rows = con.execute("SELECT ts, kind, detail FROM events ORDER BY ts DESC LIMIT ?", (limit,)).fetchall()
    con.close()
